...
*   **Contributions:** (1) Extracting subgraphs via EAP-IG and LRP. (2) Introducing *Circuit-Locked Pruning*. (3) Introducing *Mixed-Precision TCQ*. (4) Benchmarking against standard uniform baselines.
...
*   **3.3 The Gap:** Point out that while interpretability and compression are heavily researched, using interpretability to *guide* compression is a nascent, under-explored field.
...
*   **4.2 Subgraph Isolation:** Detail how you implemented EAP-IG and LRP to find the edges.
*   **4.3 Circuit Quality Metrics:** 
    *   *Faithfulness:* Define KL-Divergence and Logit Difference mathematically.
    *   *Completeness:* Explain Resample Ablation (what happens to the output when you ablate the non-circuit edges?).
    *   *Minimality:* Explain Edge Sparsity (how small did you get the subgraph while retaining faithfulness?).

#### **Chapter 5: Circuit-Aware Compression (Methodology Part 2)**
*   *This is the core of your engineering contribution.*
*   **5.1 Circuit-Locked Pruning:** Explain your algorithm. How do you generate the binary mask that protects the subgraph? Provide the formula for how $L_2$ pruning is applied *only* to the unmasked weights.
*   **5.2 Mixed-Precision Task-Circuit Quantization (TCQ):** Explain the memory management. How do you keep the subgraph at FP16/BF16 while routing the rest of the model through INT4 PTQ? (Discuss the technical implementation here—e.g., using `bitsandbytes` mixed with custom weight loading).
...
*   **6.4 Hardware & Environment:** GPU specs, PyTorch versions.
...
*   **7.2 Pruning Showdown:** Circuit-Locked Pruning vs. Standard $L_2$ Pruning. Show graphs comparing sparsity levels (%) against accuracy/Logit Diff. *Hypothesis to prove: Your method retains accuracy at much higher sparsity levels.*
*   **7.3 Quantization Showdown:** TCQ vs. Uniform INT4. Show the trade-off. Did TCQ maintain near-FP16 accuracy while achieving near-INT4 memory footprints?
*   **7.4 Memory & Speed Efficiency:** Bar charts showing VRAM usage and inference speed of the base model vs. baselines vs. your methods.

#### **Chapter 8: Conclusions and Future Work**
*   Summarize how preserving mechanistic structures allows for "smarter, not harder" compression.
*   **Future Work:** Suggest applying this to larger models (70B+), or dynamically identifying circuits at runtime (MoE style) before routing to mixed-precision layers.
